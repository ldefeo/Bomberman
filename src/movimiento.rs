...
impl Movimiento{
    ...
    pub fn moverse(&self,coord_x:usize,coord_y:usize,alcance:usize,mut laberinto: &mut Laberinto,estado:i32,enemigos_impactados: &mut Vec<(usize,usize)>){
        ...
        match &self {
            Movimiento::Derecha => {Movimiento::mover(coord_x,coord_y,alcance,&mut laberinto,1,0,estado,enemigos_impactados);},
            Movimiento::Izquierda => {Movimiento::mover(coord_x,coord_y,alcance,&mut laberinto,-1,0,estado,enemigos_impactados);},
            Movimiento::Abajo => {Movimiento::mover(coord_x,coord_y,alcance,&mut laberinto,0,1,estado,enemigos_impactados);},
            Movimiento::Arriba => {Movimiento::mover(coord_x,coord_y,alcance,&mut laberinto,0,-1,estado,enemigos_impactados);},
        }
    }
